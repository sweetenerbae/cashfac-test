...
В продукте используется GLM-5.2 через API Z.ai. Модель переписывает выбранную новость в заданной интонации, но не создаёт новости и не изменяет источник данных. Реальные публикации загружаются из The Guardian.

При разработке использовался Codex как вспомогательный инструмент: для проверки решений, рефакторинга, поиска граничных случаев и подготовки части документации. Архитектура, интеграции, бизнес-логика и итоговая проверка приложения выполнялись и контролировались разработчиком.
